...
| 15 | 开发协作与预置公共文件 | 公共文件说明 / 多人协作开发说明 / PR 审查提示词 |
| 16 | 基于 Coolify 的容器部署方案 | 4 容器生产环境 + PR 预览 + 大班并发策略 |
| 17 | 学生实践手册 | 17.1 教学常见问题 / 17.2 Commit 规范 / 17.3 开发环境搭建 / 17.4 Gitflow规范 |
| 18 | 推荐软件开发方法：Superpowers | copilot-enhance + HackLLM Memory + 循环协作工作流 |
| 19 | 软件测试方案 | UI 测试 / 功能测试 / 基于 Superpowers 的 TDD 工作流 |
| 20 | 工程实践文档要求 | 开题报告 / 需求分析 / 概要设计 / 详细设计 / 编码 / 测试 / 答辩 PPT / 总结报告 |
...
本节说明学生在开发过程中需要使用的三类核心文档与预置公共文件，包括文件位置、用途、使用方法和示例。

### 15.1 预置公共文件说明

为保障 43 名学生开发的一致性，项目在 `oaepp/` 目录下预置了以下公共文件。学生**可以直接使用**这些文件中的类、函数、常量，**不得修改**这些文件的核心逻辑（如需扩展，请在对应文件末尾追加，并通知团队同步）。

#### 15.1.1 文件清单

| 文件路径 | 文件类型 | 用途 | 学生操作 |
|----------|----------|------|----------|
| `oaepp/routes.json` | JSON 配置文件 | 原型文件与 Reflex 页面的路由映射表 | **只读**：查看原型文件对应哪个页面路由 |
| `oaepp/states/__init__.py` | Python 模块 | `GlobalState` 基类，所有页面 State 的继承父类 | **继承使用**：新建页面 State 时继承 `GlobalState` |
| `oaepp/constants.py` | Python 模块 | 全局常量、枚举类型（考勤状态、考试类型、题目类型等） | **导入使用**：`from oaepp.constants import ATTENDANCE_STATUS` |
| `oaepp/utils/__init__.py` | Python 模块 | 工具函数统一导出 | **导入使用**：`from oaepp.utils import validate_student_no` |
| `oaepp/utils/helpers.py` | Python 模块 | 通用工具函数（验证、格式化、URL 生成等） | **导入使用**：见下方示例 |

#### 15.1.2 使用示例

**示例 1：新建页面 State 时继承 `GlobalState`**

```python
# 在 pages/dashboard/state.py 中
...
> 继承后自动获得 `current_user`、`is_loading`、`show_toast()` 等公共属性和方法。

**示例 2：使用常量枚举**

```python
# 在 pages/attendance/state.py 中
from oaepp.constants import ATTENDANCE_STATUS

class AttendanceState(GlobalState):
    status_options: list[str] = [s.value for s in ATTENDANCE_STATUS]
    # 结果：["present", "absent", "late", "excused"]
```

**示例 3：使用工具函数**

```python
# 在 pages/profile/state.py 中
from oaepp.utils import validate_student_no, format_score

class ProfileState(GlobalState):
    def submit_student_no(self, student_no: str):
        if not validate_student_no(student_no):
            self.show_toast("学号必须为 10 位数字", "error")
            return
        
        self.show_toast("学号验证通过", "success")

    def display_score(self, raw_score: float):
...
```bash
# 查看原型文件 "3-课程学习.html" 对应哪个路由
cat oaepp/routes.json | grep "课程学习"
# 输出：{"prototype": "3-课程学习.html", "route": "/courses", "page": "courses", "state": "CoursesState"}
```
...
| 文件 | 可否修改 | 说明 |
|------|----------|------|
| `routes.json` | ❌ 禁止 | 路由映射由团队统一维护，个人不得自行添加 |
| `states/__init__.py` | ❌ 禁止 | `GlobalState` 基类由团队统一维护 |
| `constants.py` | ⚠️ 可追加 | 可在文件末尾追加新枚举，但需通知团队同步 |
| `utils/helpers.py` | ⚠️ 可追加 | 可在文件末尾追加新工具函数，但需通知团队同步 |
| `utils/__init__.py` | ⚠️ 可追加 | 追加新导出函数时同步更新 |

> **原则**：公共文件影响所有学生，修改需经团队共识。个人功能代码应写在 `pages/` 目录下自己的文件中。

### 15.2 多人协作开发说明使用说明

**文件位置**：`oaepp/Reflex框架一个功能模块的多人协作开发说明.md`

**适用对象**：所有学生开发者

**用途**：指导学生在 Reflex 框架下，如何多人协作开发同一个功能模块（如「课程学习」模块由 3 名学生分别负责不同页面）。

**核心内容**：

| 章节 | 内容 | 阅读优先级 |
|------|------|-----------|
| 项目结构 | `pages/` 目录隔离原则，每个子目录一个功能模块 | **必读** |
| 路由配置 | `app.py` 路由注册流程，`routes.json` 映射规则 | **必读** |
| 文件命名规范 | 原型文件与页面文件的命名对应关系 | **必读** |
| State 继承 | `GlobalState` 继承模式，页面 State 编写规范 | **必读** |
| 预置公共文件 | `constants.py`、`utils/helpers.py` 等文件的使用说明 | 推荐阅读 |
| 本地测试 | `reflex run` 启动、热重载、调试方法 | **必读** |
| 提交与 PR | 分支命名、Commit 格式、PR 描述模板 | **必读** |

**使用方法**：

1. **开发前**：阅读「项目结构」和「文件命名规范」章节，确认自己负责的文件范围
2. **编写代码时**：参考「State 继承」和「路由配置」章节，确保代码符合规范
3. **提交前**：阅读「提交与 PR」章节，检查分支命名和 Commit 格式
4. **遇到问题时**：查阅「本地测试」章节的调试方法

> **注意**：本说明与 `PR_Review提示词.md` 配合使用。学生按本说明开发，教师/TA 按提示词审查 PR。

### 15.3 PR 审查提示词使用说明

**文件位置**：`oaepp/PR_Review提示词.md`

**适用对象**：教师、助教、PR 审查者

**用途**：为 PR 审查提供结构化的审查清单和自动化提示词模板，确保每次 PR 审查覆盖所有关键维度。

**核心内容**：

| 章节 | 内容 | 审查维度 |
|------|------|----------|
| 目录规则 | 学生只能修改 `pages/` 目录，命名必须匹配原型文件 | **目录合规性** |
| 分支规则 | 分支命名格式 `feature/姓名-功能名` | **分支合规性** |
| 文件规则 | 禁止修改 `backend/` 目录、禁止创建新全局文件 | **文件边界** |
| 审查流程 | 7 步审查流程：目录 → 命名 → 代码 → 测试 → Commit → PR 描述 | **完整流程** |
| 审查清单 | 逐项检查表，支持打勾标记 | **可操作清单** |
| 常见违规 | 典型违规案例与处理建议 | **参考案例** |
| PR Preview URL | 预览环境 URL 生成规则与表格输出格式 | **环境验证** |

**使用方法**：

1. **收到 PR 通知时**：打开本文件，按「审查流程」章节的 7 个步骤逐项检查
2. **目录合规性检查**：对照「目录规则」，确认学生代码在 `pages/` 下，命名匹配原型
3. **代码质量审查**：使用「审查清单」逐项打勾，记录问题
4. **生成审查意见**：将检查结果填入 PR 评论模板，给出 Approve / Request Changes
5. **PR Preview 验证**：按「PR Preview URL」章节生成预览地址，在浏览器中验证功能
...
| 项目 | 内容 |
|------|------|
| PR | #120 feat(课程学习): 新增课程列表页面 |
| 分支 | `feature/张三-课程列表` ✅ |
| 文件 | `pages/courses/list.py` ✅ |

### 审查结果
- [x] 目录合规：代码在 `pages/courses/` 下
- [x] 命名合规：`list.py` 匹配原型 `3-课程学习-列表.html`
- [x] 未修改禁止目录
- [ ] 测试覆盖：缺少单元测试 ❌
- [x] Commit 格式规范
- [ ] PR 描述完整 ❌
...
### 意见
1. 请补充 `tests/test_courses_list.py` 单元测试
2. PR 描述请补充变更说明和测试情况
```

> **教师提示**：本提示词文件可通过 GitHub Copilot 直接读取。在 VS Code 内打开 PR 时，将本文件内容粘贴到 Copilot Chat 中，可自动生成审查报告。
...
> **⚠️ 重要注意事项：域名命名规范与 Let's Encrypt 证书问题**
>
> 在 Coolify 中设置 Project 时，**严禁为资源配置包含下划线 `_` 的域名**（例如 `oaepp_reflex.uwis.cn`、`105.oaepp_reflex.uwis.cn`、`98.oaepp_reflex.uwis.cn`）。
>
> 根据 **RFC 1035** 域名规范要求：域名仅允许包含字母、数字和连字符 `-`，不能有下划线 `_`。Let's Encrypt 严格执行此规则，不允许为包含下划线的域名颁发证书，导致 Caddy/Traefik 代理持续申请证书并反复失败，最终造成代理服务异常，无法正常响应合法域名的请求（如 `coolify.uwis.cn`）。
>
> **正确做法**：域名中仅使用字母、数字和连字符 `-`，例如 `oaepp-reflex.uwis.cn`。
>
> **⚠️ 容器端口配置注意事项**
>
> 1. **docker-compose.yaml 端口配置**：在项目的 `docker-compose.yaml` 文件中，**不要绑定主机端口**（如 `ports: - "8000:8000"`），仅使用 `expose` 声明端口即可。如果固定绑定主机端口，会在 PR Preview 时造成 PR 容器与 main 分支容器端口冲突。
>
> 2. **数据库配置**：整个项目应使用统一的数据库，通过 Coolify Project 中的 Database 资源进行管理，**不要绑定数据库主机端口**，仅启用 Database 的「Public Available」功能即可。
...
#### 16.7.3 配置 git commit 自动部署（Webhook）
...
本节汇总工程实践 4 课程中学生需要掌握的三类实践规范：教学常见问题应对（17.1）、Git Commit 规范（17.2）、本机开发环境搭建（17.3）。

### 17.1 教学常见问题与平台应对策略

本小节针对工程实践 4 教学中的典型问题，说明平台如何检测、预警、并支持教师采取干预措施。

#### 17.1.1 学生不认领 Issue（未参与实践）
