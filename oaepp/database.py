...
# ── 兼容接口：get_db() / init_table() ──────────────────────────
# 为 models/notice.py 和 routers/notice.py 提供兼容的同步接口

@contextmanager
def get_db():
    """兼容旧版 API 的同步数据库连接上下文。

    返回一个 pymysql 连接对象（带 DictCursor），支持 .cursor() 和 .commit()。
...
def init_table(table_name: str, create_ddl: str):
    """初始化数据库表（幂等操作）。
    ...
    try:
        ...
        logger.warning(f"表 {table_name} 初始化失败: {e}")
    finally:
        conn.close()
