...
路由: /score_dashboard（由 app.py 自动发现注册）
页面函数: score_dashboard_page()
    ...
    return rx.card(
        rx.hstack(
            rx.vstack(
                ...
                rx.select(
                    ScoreDashboardState.course_labels,
                    placeholder="选择课程...",
                    value=ScoreDashboardState.selected_course_label,
                    on_change=ScoreDashboardState.select_course_by_label,
                    size="2",
                    min_width="220px",
                ),
                spacing="1",
...
def _score_card(
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.heading(score, size="6", color=color),
            rx.text(
                rx.text.span("满分 "),
...
def _summary_cards() -> rx.Component:
    ...
    return rx.hstack(
        rx.card(
            rx.vstack(
                rx.text("综合总分", font_size="xs", color="gray.400",
                        font_weight="medium"),
                rx.heading(
                    ScoreDashboardState.total_score,
                    size="7",
                    color="#3b82f6",
                ),
        ...
        _score_card("出勤得分", ScoreDashboardState.attendance_score,
                     ScoreDashboardState.attendance_max, "#22c55e"),
        _score_card("考试得分", ScoreDashboardState.exam_score,
                     ScoreDashboardState.exam_max, "#a855f7"),
        _score_card("代码得分", ScoreDashboardState.code_score,
                     ScoreDashboardState.code_max, "#f97316"),
        _score_card("PR 得分", ScoreDashboardState.pr_score,
                     ScoreDashboardState.pr_max, "#3b82f6"),
        width="100%",
        spacing="4",
        wrap="wrap",
    )
...
def _radar_chart() -> rx.Component:
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ScoreDashboardState.radar_data.length() > 0,
                rx.recharts.radar_chart(
                    ...
                    data=ScoreDashboardState.radar_data,
                    width="100%",
                    height=280,
                ),
                rx.center(
...
def _bar_chart() -> rx.Component:
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ScoreDashboardState.exam_history.length() > 0,
                rx.recharts.bar_chart(
                    ...
                    rx.recharts.tooltip(),
                    data=ScoreDashboardState.exam_history,
                    width="100%",
                    height=250,
                ),
                rx.center(
...
def _line_chart() -> rx.Component:
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ScoreDashboardState.total_score_trend.length() > 0,
                rx.recharts.line_chart(
                    ...
                    rx.recharts.cartesian_grid(stroke_dasharray="3 3"),
                    rx.recharts.tooltip(),
                    data=ScoreDashboardState.total_score_trend,
                    width="100%",
                    height=250,
                ),
                rx.center(
        ...
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.vstack(
                    ...
                    rx.text(
                        (ScoreDashboardState.total_tasks
                         - ScoreDashboardState.completed_count).to(str)
                        + " 个待完成",
                        font_size="xs",
                        color="orange.500",
...
def _upcoming_deadlines_card() -> rx.Component:
    ...
    return rx.card(
        rx.vstack(
            ...
            rx.cond(
                ScoreDashboardState.upcoming_deadlines.length() > 0,
                rx.vstack(
                    rx.foreach(
                        ScoreDashboardState.upcoming_deadlines,
                        _task_row,
                    ),
                    width="100%",
                ),
...
def score_dashboard_page() -> rx.Component:
    """学生端 — 得分看板（雷达图 + 柱状图 + 折线图 + 任务完成率）。

    路由: /score_dashboard（app.py 自动发现注册）
    """
    content = rx.vstack(
        # 课程筛选
        _filter_bar(),
        ...
        margin="0 auto",
        spacing="4",
        on_mount=ScoreDashboardState.on_mount,
    )
