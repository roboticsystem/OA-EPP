...
注意：以下为普通类（不继承 rx.State），不会被 Reflex 自动发现，需显式 import 使用：
  - DeadlineState → states/deadline.py（F-S-022 截止规则）
  - CIState       → states/devops_ci.py（F-D-004 CI 自动化）
"""
...
# F-D-004 CI 自动化状态（普通类，不继承 rx.State，按需显式 import）
try:
    from .devops_ci import CIState
except Exception:
    CIState = None

__all__ = ["GlobalState", "CIState"]
