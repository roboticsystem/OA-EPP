"""F-T-002 VS Code 扩展与 Copilot 提示词配置 — Reflex State

需求编号：F-T-002
模块定位：教师端 VS Code 扩展推荐/禁止管理 + Copilot 指令文件在线编辑 + Git 一键提交

规则遵循（PR_Review提示词.md）：
- ✅ 仅创建 states/ 文件，不碰全局文件
- ✅ 继承 GlobalState，使用 show_toast 做操作反馈
- ✅ 使用 constants.py 中已定义的常量（不新增全局常量）
"""
...
if rx is not None:
    ...
    _CONFIG_PATH = os.path.join(_OAEPP_DIR, ".oaepp_data", "vscode_config.json")
    ...
    class VSCodeConfigState(GlobalState):
        """F-T-002 VS Code 扩展与 Copilot 提示词配置管理"""
        ...
        def set_rec_id(self, value: str):
            self.rec_id = value

        def set_rec_name(self, value: str):
            self.rec_name = value

        def set_ban_id(self, value: str):
            self.ban_id = value

        def set_ban_name(self, value: str):
            self.ban_name = value
        ...
        def add_recommendation(self):
            ...
            existing = any(
                e["id"] == self.rec_id
                for e in config.get("recommendations", [])
            )
            if not existing:
                config.setdefault("recommendations", []).append({
                    "id": self.rec_id,
                    "name": self.rec_name or self.rec_id,
                    "description": "",
                })
                _save_config(config)
            ext_id = self.rec_id
            ...
            self.show_toast("已添加 " + ext_id, "success")

        def remove_recommendation(self, ext_id: str):
            config = _load_config()
            config["recommendations"] = [
                e for e in config.get("recommendations", [])
                if e["id"] != ext_id
            ]
            _save_config(config)
            self.load_extensions()
            self.show_toast("已移除 " + ext_id, "success")
        ...
        def add_banned(self):
            ...
            existing = any(
                e["id"] == self.ban_id
                for e in config.get("unwantedRecommendations", [])
            )
            if not existing:
                config.setdefault("unwantedRecommendations", []).append({
                    "id": self.ban_id,
                    "name": self.ban_name or self.ban_id,
                    "description": "",
                })
                _save_config(config)
            ext_id = self.ban_id
            ...
            self.ban_name = ""
            self.load_extensions()
            self.show_toast("已禁止 " + ext_id, "success")

        def remove_banned(self, ext_id: str):
            config = _load_config()
            config["unwantedRecommendations"] = [
                e for e in config.get("unwantedRecommendations", [])
                if e["id"] != ext_id
            ]
            _save_config(config)
            self.load_extensions()
            self.show_toast("已解除禁止 " + ext_id, "success")
        ...
        def apply_preset(self, preset_id: str):
            ...
            _save_config(config)
            self.load_extensions()
            self.show_toast("已应用模板: " + preset["name"], "success")

        def generate_extensions_json(self):
            config = _load_config()
            output = {
                "recommendations": [
                    e["id"] for e in config.get("recommendations", [])
                ],
                "unwantedRecommendations": [
                    e["id"] for e in config.get("unwantedRecommendations", [])
                ],
            }
            ...
            self.show_toast("已生成 .vscode/extensions.json", "success")

        # ── Copilot 指令文件 ──

        def _save_copilot_instructions(self, path, content):
        ...
        def save_file_content(self):
            if not self.current_file or not self.editor_content:
                return
            self._save_copilot_instructions(self.current_file, self.editor_content)
            self.show_toast("已保存 " + self.current_file, "success")
        ...
        def commit_and_push(self):
            try:
                ...
                result = subprocess.run(
                    [
                        "git", "commit", "-m",
                        "chore: update VSCode & Copilot config (" + date_str + ")",
                    ],
                    cwd=_OAEPP_DIR,
                    capture_output=True,
                    text=True,
                ...
                self.git_changed_files = []
                if sha:
                    self.show_toast("提交成功: " + sha, "success")
                else:
                    self.show_toast("无变更需要提交", "info")
            except subprocess.CalledProcessError as e:
                self.show_toast("Git 操作失败: " + str(e.stderr), "error")

        # ── 页面挂载初始化 ──

        def on_mount(self):
            self.load_extensions()
            self.load_instruction_files()
            self.check_git_status()
